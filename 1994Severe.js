 ...
 *     Button always shows:  "☀ DAY — AUTO"  |  "🌙 NIGHT — AUTO"
 *                       or  "☀ DAY — MANUAL" |  "🌙 NIGHT — MANUAL"
 * · Seamless alert ticker — only visible when active alerts exist
 * · 90s Unicode weather icon set
...
═══════════════════════════════════════════ */
const DAY_META   = '#0d0d50';
const NIGHT_META = '#020214';
let themeMode = 'auto';
...
function loadTheme() {
  const saved = localStorage.getItem('cwn94_theme');
...
togBtn.addEventListener('click', () => {
  ...
  themeMode = 'manual';
  applyTheme(!night);
  localStorage.setItem('cwn94_theme', !night ? 'night' : 'day');
  updateBtn();
});
...
togBtn.addEventListener('dblclick', e => {
  e.preventDefault();
  themeMode = 'auto';
  localStorage.removeItem('cwn94_theme');
  applyTheme(isNightHour());
  updateBtn();
});
...
/* ═══════════════════════════════════════════
   ...
   Text doubled in JS → CSS animates 0 → -50%
   Duration scales with length for constant speed
═══════════════════════════════════════════ */
const PX_PER_SEC = 90;
...
async function fillCities() {
  try {
    ...
    console.error('[CWN 1994] fillCities:', err);
